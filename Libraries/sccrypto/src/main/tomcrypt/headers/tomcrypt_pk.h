...
/**
 * @file ecc_bl.h
 * @brief EC Crypto for Bernstein/Lange curves
 * @author Werner Dittmann <Werner.Dittmann@t-online.de>
 *
 * An application shall use the high-level functions only. The names of the high-level
 * functions have the prefix @c ecc_bl and use the same pattern as the ECC high-level
 * functions for the NIST curves.
 */
...
/**
 * @brief Import an ANSI X9.63 format public key
...
/**
 * @brief Decrypt an ECC encrypted key
 ...
 */
int ecc_bl_decrypt_key(const unsigned char *in,  unsigned long  inlen, unsigned char *out, unsigned long *outlen, ecc_key *key);

/**
 * @brief Encrypt a symmetric key with ECC Curve3617
 ...
 @return CRYPT_OK if successful
 */
int ecc_bl_encrypt_key(const unsigned char *in, unsigned long inlen, unsigned char *out,  unsigned long *outlen,
                       prng_state *prng, int wprng, int hash, ecc_key *key);
/**
 * @brief Import an ECC key from a binary packet
 ...
 * @brief Sign a message digest
 ...
 @return CRYPT_OK if successful
 */
int ecc_bl_sign_hash(const unsigned char *in,  unsigned long inlen,  unsigned char *out, unsigned long *outlen, prng_state *prng, int wprng, ecc_key *key);
 ...
 */
int ecc_bl_verify_hash(const unsigned char *sig,  unsigned long siglen, const unsigned char *hash, unsigned long hashlen, int *stat, ecc_key *key);
...
/*
 * Internal functions. Use them only if you know what you are doing :-)
 */
...
extern const ltc_ecc_set_type ltc_ecc_bl_sets[];

/**
 * @brief Check a public key for Curve3617.
 *
 * The method checks if a public key is valid.
 *
...
/**
 @brief Add two ECC points for Curve3617.
 
 @param P        The point to add
 @param Q        The point to add
 @param R        [out] The destination of the double
 @param modulus  The modulus of the field the ECC curve is in
 @param b        The constant of the equation (3617)
 @return CRYPT_OK on success
 */
int ltc_ecc_bl_projective_add_point(ecc_point *P, ecc_point *Q, ecc_point *R, void *modulus, void *b);

/**
 @brief Map a projective jacbobian point back to affine space - Curve3617.
...
/**
 @brief Double an ECC point for Curve3617.
...
/**
 @brief Perform a point multiplication for Curve3617.
