...
@synthesize secondaryResultsArray = _secondaryResultsArray;
...
- (void)setSecondaryResultsArray:(NSArray *)secondaryResultsArray {
	...
	[self setSearchResultsVisible:(([_resultsArray count] > 0) || ([_secondaryResultsArray count] > 0))];
...
- (CGFloat)tableView:(UITableView *)tableView heightForHeaderInSection:(NSInteger)section {
	if ([_tokenField.delegate respondsToSelector:@selector(tokenField:resultsTableView:heightForHeaderInSection:)])
		return [_tokenField.delegate tokenField:_tokenField resultsTableView:tableView heightForHeaderInSection:section];

	return 0;
}

- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
	if ([_tokenField.delegate respondsToSelector:@selector(tokenField:resultsTableView:viewForHeaderInSection:)])
		return [_tokenField.delegate tokenField:_tokenField resultsTableView:tableView viewForHeaderInSection:section];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	switch (indexPath.section) {
		case 0:
			representedObject = [_resultsArray objectAtIndex:indexPath.row];
			break;
		case 1:
			if (_secondaryResultsArray)
				representedObject = [_secondaryResultsArray objectAtIndex:indexPath.row];
			break;
	}
...
- (void)tableView:(UITableView *)tableView willDisplayCell:(UITableViewCell *)cell forRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	if ([_tokenField.delegate respondsToSelector:@selector(tokenField:resultsTableView:willDisplayCell:forRepresentedObject:)]) {
		[_tokenField.delegate tokenField:_tokenField resultsTableView:tableView willDisplayCell:cell forRepresentedObject:representedObject];
		return;
	}
	
	[cell.textLabel setText:[self searchResultStringForRepresentedObject:representedObject]];
	[cell.detailTextLabel setText:[self searchResultSubtitleForRepresentedObject:representedObject]];
}
...
- (void)searchForSecondaryResultsWithString:(NSString *)searchString {
	if ([_tokenField.delegate respondsToSelector:@selector(tokenField:searchForSecondaryResultsWithString:)]){
		[_tokenField.delegate tokenField:_tokenField searchForSecondaryResultsWithString:searchString];
	}
}
...
- (void)resultsForSearchString:(NSString *)searchString {
	...
	if (searchString.length || _forcePickSearchResult){
		...
		[self searchForSecondaryResultsWithString:searchString];
	}
	...
	if (_secondaryResultsArray.count > 0) {
		[_secondaryResultsArray sortUsingComparator:^NSComparisonResult(id obj1, id obj2) {
			return [[self searchResultStringForRepresentedObject:obj1] localizedCaseInsensitiveCompare:[self searchResultStringForRepresentedObject:obj2]];
		}];
		bReload = YES;
	}
...
- (void)tokenTouchUpInside:(TIToken *)token {
    ...
    if(token.accessoryType == TITokenAccessoryTypeAlertIndicator)
    {
        if ([delegate respondsToSelector:@selector(tokenField:didHitAlertToken:)]){
			[delegate tokenField:self didHitAlertToken:token];
        }
    }
...
- (void)sizeToFit {
    ...
    else if (_accessoryType == TITokenAccessoryTypeActivityIndicator)
    {
 		accessoryWidth += 21.;
    }
    else if (_accessoryType == TITokenAccessoryTypeAlertIndicator)
    {
 		accessoryWidth += floorf(hTextPadding / 2) + 18;
    }
...
- (void)drawRect:(CGRect)rect {
	...
	CGFloat accessoryWidth = 0;
     ...
     if (_accessoryType != TITokenAccessoryTypeActivityIndicator)
     {
         if(_activity )
         {
    ...
    else  if (_accessoryType == TITokenAccessoryTypeActivityIndicator)
    {
        if(!_activity )
        {
            ...
            _activity.center =  CGPointMake(self.bounds.size.width - 14, (self.bounds.size.height / 2) - 1);
    ...
    else  if (_accessoryType == TITokenAccessoryTypeAlertIndicator)
    {
        ...
        CGContextSaveGState(context);
        UIImage * newImage = [UIImage imageNamed:@"failure-btn"];
         CGRect imageRect = CGRectMake( self.bounds.size.width - floorf(hTextPadding / 2) - 18, 0, 23, 23);
