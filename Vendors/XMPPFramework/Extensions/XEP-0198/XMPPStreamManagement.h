 ...
 * 
 * @return
 *   Returns NO if stream management is already enabled/resumed,
 *   or the process of enabling stream management has already been started.
 *   Returns YES otherwise, after sending the <enable> stanza to the server.
 *   Remember that the return value is only indicative of an immediate error.
 *   The server's response to our enable request will ultimately be reported via the delegate methods.
 *
 * @see supportsStreamManagement
**/
- (BOOL)enableStreamManagementWithResumption:(BOOL)supportsResumption maxTimeout:(NSTimeInterval)maxTimeout;
