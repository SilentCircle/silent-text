...
- (void)dealloc
{	
	for (XMPPTimer *timer in discoTimerJidDict)
	{
		[timer cancel];
	}
}
...
- (void)setupTimeoutForDiscoRequestFromJID:(XMPPJID *)jid
{
	...
	XMPPTimer *timer = [[XMPPTimer alloc] initWithQueue:moduleQueue eventHandler:^{ @autoreleasepool {
		
		[self processTimeoutWithJID:jid];
	}}];
	
	[timer startWithTimeout:CAPABILITIES_REQUEST_TIMEOUT interval:0];
	
	// We keep the timer in the discoTimerJidDict.
	// We'll need to cancel the timer if we get a response to the disco request.
...
- (void)cancelTimeoutForDiscoRequestFromJID:(XMPPJID *)jid
{
	...
	XMPPTimer *timer = [discoTimerJidDict objectForKey:jid];
	if (timer)
	{
		[timer cancel];
